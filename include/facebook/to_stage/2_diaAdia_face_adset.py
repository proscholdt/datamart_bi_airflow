"""Ingestão facebook — AD SET (API Graph → STAGE).

Insights nível adset com time_increment=1 (poucas chamadas, fatiado) + 1 chamada
do mapa de públicos personalizados por adset. 1 arquivo JSON por dia no stage.
Substitui o loop dia-a-dia (rate-limit no tier development_access). Janela do api_window.
"""
import os
import json
import time
...
def _extract_custom_aud_names_from_targeting(targeting):
    """Nomes/ids de públicos personalizados incluídos (custom_audiences + flexible_spec)."""
    if not targeting:
        return []
    names, ids = set(), set()
    ...
    def _push(lst):
        for ca in (lst or []):
            name, cid = ca.get("name"), ca.get("id")
            if name:
                names.add(name)
            elif cid:
                ids.add(cid)
    ...
    result = sorted(names)
    result += [f"CA:{cid}" for cid in sorted(ids)]
    return result


def build_adset_custom_audience_map():
    """adset_id -> string com públicos personalizados incluídos (1 chamada + paging)."""
    u = f"{G}/{AD_ACCOUNT_ID}/adsets"
    p = {"access_token": ACCESS_TOKEN, "fields": "id,name,targeting,effective_status", "limit": 200}
    mapping = {}
    while True:
        j = _get(u, p, "adsets_map")
        for adset in j.get("data", []):
            aud = _extract_custom_aud_names_from_targeting(adset.get("targeting"))
            mapping[str(adset.get("id"))] = " | ".join(aud) if aud else ""
        after = j.get("paging", {}).get("cursors", {}).get("after")
        if not after:
            break
        p = dict(p); p["after"] = after
    return mapping
...
def fetch_facebook_adsets_by_day(start_date, end_date, only_with_data=False):
    ...
    for cs, ce in _chunks(start_date, end_date):
        ...
        p = {"access_token": ACCESS_TOKEN, "time_range": json.dumps({"since": cs, "until": ce}),
             "time_increment": 1, "level": "adset", "fields": ",".join(REQUIRED_FIELDS), "limit": 500}
        while True:
            ...
            if not nxt:
                break
            u, p = nxt, {}
    print(f"insights adset: {len(rows)} linhas ({start_date}→{end_date})")
    ...
    for date_str in sorted(by_day):
        ...
        for item in items:
            ...
            aid = str(item.get("adset_id", "")) if item.get("adset_id") is not None else ""
            item["publicos_personalizados_incluidos"] = adset_custom_map.get(aid, "")
        ...
        if any("adset_id" not in d for d in data):
            raise Exception(f"❌ Validação: registro sem adset_id em {date_str}")
...
if __name__ == "__main__":
    ...
    fetch_facebook_adsets_by_day(start_date, end_date, only_with_data=True)
